...
% passer les QUESTIONS_... a 1 et completer les parties correspondantes  au fur et a mesure de l'avancement
% Ctrl-F [QUEstion_] permet d'atteindre rapidement la partie du programme
% de meme si une phase n'est plus utile, on peut repasser le switch a 0 pour gagner du temps
%---------------------------------------------------------------------------
QUESTION_23_simu_bo = 0; % 1 pour simuler la boucle ouverte, 0 sinon
QUESTION_24_linearise_bo = 0; % 1 pour lineariser la boucle ouverte, 0 sinon
QUESTION_31_commandabilite_vc = 0; % 1 pour analyser la commandabilite depuis vc, 0 sinon
QUESTION_commandabilite_grammien = 0;%
QUESTION_32_observabilite_ab = 0;
QUESTION_33_observabilite_xc_vab = 0;
QUESTION_41a_calcule_K_LQR = 0;
QUESTION_41c_linearise_LQR = 0;
QUESTION_41c_trace_bode_LQR = 0;
QUESTION_41d_simu_LQR = 0;
QUESTION_52_simule_estimateur_gain_nul = 0;
QUESTION_53c_calcule_LQE = 0;
QUESTION_53d_linearise_LQR_LQE = 0;
QUESTION_62_linearise_LQG = 0;
QUESTION_62_trace_bodeLQG = 0;
QUESTION_63_simule_LQG = 0;


%% --------------------------------------------------------------
% declaration des variables et constantes dans la structure CH
%--------------------------------------------------------------
p = tf('p');
CH = struct();
w = logspace(-2,3,1000).'; % pulsations de 10^-1 a 10^4 avec 1000 points, echelle log
...
CH.fb = 1e-4;
CH.vc_max = inf; % pas de limitation de vc par defaut, max =infini
CH.Lpend = CH.Lb;
% Moto Reducteur
CH.delayVc = 0 ; % par defaut on ne prend pas en compte le retard du hacheur
p = tf('p') ; % creation fct de transfert p
tfDelayVc = (1-CH.delayVc/2*p)/(1+CH.delayVc/2*p); % fct transfert approx de Pade du retard
CH.ssDelayVc = ss(tfDelayVc) ; %conversion sous forme representation d'etat
CH.KH = 12/5;
CH.KE = 7;
%%  entrees (blocs from workspace)
...
CH.valphab_init = 0;
%% inports indiques de facon lisible
CH.in_Fxcp = 1;
CH.in_Cb = 2;
CH.in_vc = 3;
CH.in_refxc = 4;
%% outports indiques de facon lisible
CH.out_xc = 1;
CH.out_alphab = 2;
CH.out_vxc = 3;
CH.out_valphab = 4;
